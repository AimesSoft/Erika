...
const NATIVE_BUFFER_USAGE_HW_TEXTURE: u64 = 1 << 9;
const OUTPUT_VK_FORMAT: vk::Format = vk::Format::R16G16B16A16_SFLOAT;
const MAX_PENDING_NATIVE_BUFFER_CONVERSIONS: usize = 3;
const OHOS_NATIVE_BUFFER_HANDLE_TYPE: vk::ExternalMemoryHandleTypeFlags =
    vk::ExternalMemoryHandleTypeFlags::from_raw(0x0000_2000);
const OHOS_EXTERNAL_MEMORY_EXTENSION_NAME: &CStr = c"VK_OHOS_external_memory";
const OHOS_NATIVE_BUFFER_EXTENSION_NAME: &CStr = c"VK_OHOS_native_buffer";
...
type GetNativeBufferPropertiesOhos = unsafe extern "system" fn(
    vk::Device,
    *const c_void,
    *mut VkNativeBufferPropertiesOhos,
) -> vk::Result;

struct OhosExternalMemoryFunctions {
    get_native_buffer_properties: GetNativeBufferPropertiesOhos,
}

impl OhosExternalMemoryFunctions {
    unsafe fn load(instance: &ash::Instance, device: vk::Device) -> Result<Self, String> {
        let address = unsafe {
            instance.get_device_proc_addr(device, c"vkGetNativeBufferPropertiesOHOS".as_ptr())
        }
        .ok_or_else(|| {
            "VK_OHOS_external_memory did not expose vkGetNativeBufferPropertiesOHOS".to_string()
        })?;
        Ok(Self {
            get_native_buffer_properties: unsafe {
                mem::transmute::<unsafe extern "system" fn(), GetNativeBufferPropertiesOhos>(
                    address,
                )
            },
        })
...
pub(crate) enum OhosNativeBufferConversionError {
    Backpressure { pending: usize, max: usize },
    Interop(String),
}

impl fmt::Display for OhosNativeBufferConversionError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Backpressure { pending, max } => write!(
                formatter,
                "stage=reserve_ohb_conversion reason=retirement_capacity_exhausted pending={pending} max={max}"
            ),
...
impl From<String> for OhosNativeBufferConversionError {
...
pub(crate) struct OhosNativeBufferCrop {
    pub left: u32,
    pub top: u32,
    pub right: u32,
    pub bottom: u32,
...
/// A decoded OH_NativeBuffer that is ready for Vulkan access.
///
...
pub(crate) struct OhosNativeBufferFrameDescription {
    pub native_buffer: NonNull<c_void>,
    pub buffer_width: u32,
    pub buffer_height: u32,
    pub usage: u64,
    pub crop: OhosNativeBufferCrop,
    /// Size of the intermediate RGB texture. This may be smaller than the
    /// decoded crop when the presentation surface is smaller than the video.
...
pub(crate) struct OhosNativeBufferConversion {
    pub texture: wgpu::Texture,
    pub width: u32,
    pub height: u32,
    /// Must be retained until the queue submission containing the conversion has completed.
    pub pending: PendingOhosNativeBufferConversion,
}
...
pub(crate) struct PendingOhosNativeBufferConversion {
    device: ash::Device,
    ...
    _cached: Arc<CachedOhosNativeBufferConversion>,
    owner: Option<Arc<dyn Any + Send + Sync>>,
}

impl PendingOhosNativeBufferConversion {
    fn new(
        interop: &OhosVulkanInterop,
        owner: Arc<dyn Any + Send + Sync>,
        cached: Arc<CachedOhosNativeBufferConversion>,
...
impl Drop for PendingOhosNativeBufferConversion {
    fn drop(&mut self) {
        unsafe {
...
pub(crate) fn retire_ohb_conversion_after_submission(
    queue: &wgpu::Queue,
    pending: PendingOhosNativeBufferConversion,
) {
...
impl ConversionCacheKey {
    fn new(properties: &NativeBufferProperties, parameters: ConversionParameters) -> Self {
        Self {
...
struct CachedOhosNativeBufferConversion {
...
impl CachedOhosNativeBufferConversion {
    fn empty(
        interop: &OhosVulkanInterop,
...
impl Drop for CachedOhosNativeBufferConversion {
    fn drop(&mut self) {
        unsafe {
...
pub(crate) struct OhosVulkanDeviceContext {
    pub instance: wgpu::Instance,
    pub adapter: wgpu::Adapter,
    pub device: wgpu::Device,
    pub queue: wgpu::Queue,
    pub supports_16bit_norm: bool,
    pub interop: OhosVulkanInterop,
}

pub(crate) struct OhosVulkanInterop {
    pub(crate) instance: ash::Instance,
    pub(crate) device: ash::Device,
    pub(crate) physical_device: vk::PhysicalDevice,
    pub(crate) queue_family_index: u32,
    external_memory: OhosExternalMemoryFunctions,
    conversion_cache: Mutex<HashMap<ConversionCacheKey, Arc<CachedOhosNativeBufferConversion>>>,
...
impl OhosVulkanInterop {
    ...
    /// # Safety
    ///
    /// `frame.native_buffer` must point to a live AHardwareBuffer whose acquire fence has already
    ...
    pub(crate) unsafe fn convert_native_buffer(
        ...
        let properties = unsafe { self.query_native_buffer_properties(native_buffer)? };
        validate_ohb_format(&properties)?;
        ...
        let mut pending =
            PendingOhosNativeBufferConversion::new(self, owner, Arc::clone(&cached), reservation);
        ...
        unsafe {
            self.device
                ...
                .map_err(|error| vk_stage_error("bind_imported_ohb_memory", error))?;
        }
        ...
        let output = wgpu_device.create_texture(&wgpu::TextureDescriptor {
            label: Some("erika-ohos-ohb-rgba16f"),
            size: wgpu::Extent3d {
                width: output_width,
        ...
        let record_result = unsafe {
            conversion_encoder.as_hal_mut::<wgpu::hal::vulkan::Api, _, _>(|hal_encoder| {
                let hal_encoder = hal_encoder.ok_or_else(|| {
                    "stage=record_ohos_ohb_conversion reason=not_vulkan".to_string()
                })?;
                let command_buffer = hal_encoder.raw_handle();
                if command_buffer.is_null() {
                    return Err(
                        "stage=record_ohos_ohb_conversion reason=no_active_command_buffer"
                            .to_string(),
                    );
                }
    ...
    fn validate_wgpu_device(&self, device: &wgpu::Device) -> Result<(), String> {
        let hal_device = unsafe { device.as_hal::<wgpu::hal::vulkan::Api>() }
            .ok_or_else(|| "stage=validate_ohos_vulkan_device reason=not_vulkan".to_string())?;
        ...
        {
            return Err(
                "stage=validate_ohos_vulkan_device reason=interop_device_mismatch".to_string(),
            );
        }
        Ok(())
    }
    ...
    fn reserve_pending_conversion(
        &self,
    ) -> Result<PendingConversionReservation, OhosNativeBufferConversionError> {
        let mut current = self.pending_conversions.load(Ordering::Acquire);
        loop {
            if current >= MAX_PENDING_NATIVE_BUFFER_CONVERSIONS {
                return Err(OhosNativeBufferConversionError::Backpressure {
                    pending: current,
                    max: MAX_PENDING_NATIVE_BUFFER_CONVERSIONS,
                });
            }
    ...
    unsafe fn allocate_and_write_descriptor_set(
        ...
        loop {
            let descriptor_pool = unsafe { create_descriptor_pool(&self.device, budget) }.map_err(
                |error| {
                    format!(
                        "stage=create_ohb_descriptor_pool reason=vulkan_error code={error:?} budget={budget} limit={limit} source={} externalFormat={external_format}",
            ...
            match unsafe {
                ...
                Ok(descriptor_set) => {
                    ...
                    if budget > previous {
                        crate::trace::diagnostic(
                            serde_json::json!({
                                "event": "ohos_ohb_descriptor_budget",
                                "stage": "active",
                                "budget": budget,
                ...
                Err(error) => {
                    ...
                    return Err(format!(
                        "stage=allocate_ohb_descriptor_set reason=vulkan_error code={error:?} budget={budget} limit={limit} source={} externalFormat={external_format}",
                        self.descriptor_budget_source
                    ));
                }
            }
    ...
    unsafe fn cached_conversion(
        &self,
        wgpu_device: &wgpu::Device,
        properties: &NativeBufferProperties,
        parameters: ConversionParameters,
    ) -> Result<Arc<CachedOhosNativeBufferConversion>, String> {
        let key = ConversionCacheKey::new(properties, parameters);
        let mut cache = self
            .conversion_cache
            .lock()
            .map_err(|_| "stage=lock_ohb_conversion_cache reason=mutex_poisoned".to_string())?;
        ...
        let mut cached = CachedOhosNativeBufferConversion::empty(
            self,
            wgpu_device,
        ...
        cached.vertex_shader = unsafe {
            create_shader_module(
                &self.device,
                include_bytes!("ohos_native_buffer.vert.spv"),
                "create_ohos_ohb_vertex_shader",
            )?
        };
        cached.fragment_shader = unsafe {
            create_shader_module(
                &self.device,
                include_bytes!("ohos_native_buffer.frag.spv"),
                "create_ohos_ohb_fragment_shader",
            )?
        };
    ...
    unsafe fn query_native_buffer_properties(
        &self,
        native_buffer: NonNull<c_void>,
    ) -> Result<NativeBufferProperties, String> {
        let mut format = VkNativeBufferFormatPropertiesOhos {
            s_type: vk::StructureType::from_raw(1_000_452_002),
            p_next: std::ptr::null_mut(),
            format: vk::Format::UNDEFINED,
            external_format: 0,
            format_features: vk::FormatFeatureFlags::empty(),
            sampler_ycbcr_conversion_components: vk::ComponentMapping::default(),
            suggested_ycbcr_model: vk::SamplerYcbcrModelConversion::RGB_IDENTITY,
        ...
        let mut properties = VkNativeBufferPropertiesOhos {
            s_type: vk::StructureType::from_raw(1_000_452_001),
            p_next: std::ptr::from_mut(&mut format).cast(),
            allocation_size: 0,
            memory_type_bits: 0,
        };
        let result = unsafe {
            (self.external_memory.get_native_buffer_properties)(
                self.device.handle(),
                native_buffer.as_ptr().cast_const(),
                &mut properties,
            )
        };
        if result != vk::Result::SUCCESS {
            return Err(vk_stage_error("query_native_buffer_properties", result));
        }
        let allocation_size = properties.allocation_size;
        let memory_type_bits = properties.memory_type_bits;
        Ok(NativeBufferProperties {
            allocation_size,
            memory_type_bits,
    ...
    unsafe fn create_imported_image(
        ...
        let mut external = VkExternalFormatOhos {
            s_type: vk::StructureType::from_raw(1_000_452_005),
            p_next: std::ptr::null_mut(),
            external_format,
        };
        let mut external_memory = vk::ExternalMemoryImageCreateInfo::default()
            .handle_types(OHOS_NATIVE_BUFFER_HANDLE_TYPE);
        if external_format != 0 {
            external_memory.p_next = std::ptr::from_mut(&mut external).cast();
        }
        ...
        create_info.p_next = std::ptr::from_mut(&mut external_memory).cast();
        unsafe { self.device.create_image(&create_info, None) }
            .map_err(|error| vk_stage_error("create_imported_ohb_image", error))
    }
    ...
    unsafe fn allocate_imported_memory(
        ...
        let mut import = VkImportNativeBufferInfoOhos {
            s_type: vk::StructureType::from_raw(1_000_452_003),
            p_next: std::ptr::null(),
            buffer: native_buffer.as_ptr(),
        };
        let mut dedicated = vk::MemoryDedicatedAllocateInfo::default().image(image);
        dedicated.p_next = std::ptr::from_mut(&mut import).cast();
        let mut allocate_info = vk::MemoryAllocateInfo::default()
            .allocation_size(allocation_size)
            .memory_type_index(memory_type_index);
        allocate_info.p_next = std::ptr::from_mut(&mut dedicated).cast();
        unsafe { self.device.allocate_memory(&allocate_info, None) }
            .map_err(|error| vk_stage_error("allocate_imported_ohb_memory", error))
    }
    ...
    unsafe fn create_ycbcr_conversion(
        ...
        if properties.external_format != 0 {
            create_info.p_next = std::ptr::from_mut(&mut external).cast();
        }
        if via_khr {
            unsafe {
                ash::khr::sampler_ycbcr_conversion::Device::new(&self.instance, &self.device)
        ...
        .map_err(|error| vk_stage_error("create_ohb_ycbcr_conversion", error))
    }
}
...
fn validate_frame_description(
    ...
    if frame.buffer_width == 0 || frame.buffer_height == 0 {
        return Err(format!(
            "stage=validate_ohb_frame reason=zero_buffer_extent width={} height={}",
            frame.buffer_width, frame.buffer_height
        ));
    }
    if frame.usage & NATIVE_BUFFER_USAGE_HW_TEXTURE == 0 {
        return Err(format!(
            "stage=validate_ohb_frame reason=missing_gpu_sampled_usage usage=0x{:x}",
            frame.usage
        ));
    }
    let crop = frame.crop;
    ...
    {
        return Err(format!(
            "stage=validate_ohb_frame reason=invalid_crop crop={},{},{},{} buffer={}x{}",
    ...
    if frame.output_width == 0 || frame.output_height == 0 {
        return Err(format!(
            "stage=validate_ohb_frame reason=zero_output_extent width={} height={}",
    ...
    if frame.output_width > max_dimension || frame.output_height > max_dimension {
        return Err(format!(
            "stage=validate_ohb_frame reason=output_extent_exceeds_device_limit output={}x{} max={max_dimension}",
            frame.output_width, frame.output_height
        ));
    }
    Ok((
...
fn validate_ohb_format(properties: &NativeBufferProperties) -> Result<(), String> {
    if properties.allocation_size == 0 {
        return Err("stage=validate_ohb_format reason=zero_allocation_size".to_string());
    }
    if properties.memory_type_bits == 0 {
        return Err("stage=validate_ohb_format reason=no_memory_types".to_string());
    }
    if properties.external_format == 0 && properties.format == vk::Format::UNDEFINED {
        return Err(format!(
            "stage=validate_ohb_format reason=missing_external_format vkFormat={}",
    ...
    {
        return Err(format!(
            "stage=validate_ohb_format reason=not_sampleable features=0x{:x}",
...
fn validate_chroma_location(
    ...
    if supported {
        ...
        Err(format!(
            "stage=validate_ohb_format reason=unsupported_chroma_offset axis={axis} location={} features=0x{:x}",
            location.as_raw(),
            features.as_raw()
        ))
    }
...
fn conversion_parameters(
    ...
    {
        return Err(format!(
            "stage=select_ohb_ycbcr_conversion reason=unsupported_model model={}",
            model.as_raw()
        ));
    }
    ...
    if range != vk::SamplerYcbcrRange::ITU_NARROW && range != vk::SamplerYcbcrRange::ITU_FULL {
        return Err(format!(
            "stage=select_ohb_ycbcr_conversion reason=unsupported_range range={}",
            range.as_raw()
        ));
    }
...
fn choose_memory_type(
    ...
    device_local
        ...
        .ok_or_else(|| {
            format!(
                "stage=select_ohb_memory_type reason=no_compatible_type bits=0x{memory_type_bits:x} count={count}"
            )
        })
}
...
unsafe fn create_ycbcr_sampler(
    ...
    unsafe { device.create_sampler(&create_info, None) }
        .map_err(|error| vk_stage_error("create_ohb_ycbcr_sampler", error))
}
...
unsafe fn create_ycbcr_image_view(
    ...
    unsafe { device.create_image_view(&create_info, None) }
        .map_err(|error| vk_stage_error("create_imported_ohb_view", error))
}
...
unsafe fn create_descriptor_set_layout(
    ...
    unsafe { device.create_descriptor_set_layout(&create_info, None) }
        .map_err(|error| vk_stage_error("create_ohb_descriptor_set_layout", error))
}
...
unsafe fn create_pipeline_layout(
    ...
    unsafe { device.create_pipeline_layout(&create_info, None) }
        .map_err(|error| vk_stage_error("create_ohb_pipeline_layout", error))
}
...
unsafe fn create_output_render_pass(device: &ash::Device) -> Result<vk::RenderPass, String> {
    ...
    unsafe { device.create_render_pass(&create_info, None) }
        .map_err(|error| vk_stage_error("create_ohb_output_render_pass", error))
}
...
unsafe fn create_conversion_pipeline(
    ...
    match unsafe { device.create_graphics_pipelines(vk::PipelineCache::null(), &create_info, None) }
    {
        ...
        Err((pipelines, error)) => {
            ...
            Err(vk_stage_error("create_ohb_conversion_pipeline", error))
        }
    }
}
...
fn establish_wgpu_output_state(encoder: &mut wgpu::CommandEncoder, texture: &wgpu::Texture) {
    ...
    let _pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
        label: Some("erika-ohos-ohb-output-state"),
...
unsafe fn create_output_image_view(
    ...
    unsafe { device.create_image_view(&create_info, None) }
        .map_err(|error| vk_stage_error("create_ohb_output_view", error))
}
...
unsafe fn create_output_framebuffer(
    ...
    unsafe { device.create_framebuffer(&create_info, None) }
        .map_err(|error| vk_stage_error("create_ohb_output_framebuffer", error))
}
...
fn ohos_custom_vulkan_instance_flags(mut flags: wgpu::InstanceFlags) -> wgpu::InstanceFlags {
    ...
    // naming hook. Ohos Emulator's ranchu driver dereferences a null object
...
pub(crate) fn create_device() -> Result<OhosVulkanDeviceContext, String> {
    let requested_instance_flags = wgpu::InstanceFlags::from_build_config().with_env();
    let instance_flags = ohos_custom_vulkan_instance_flags(requested_instance_flags);
    if requested_instance_flags.contains(wgpu::InstanceFlags::DEBUG) {
        crate::trace::diagnostic(
            serde_json::json!({
                "event": "ohos_vulkan",
                "stage": "debug_utils_disabled",
                "backendCandidate": "vulkan-ohb",
    ...
    if !missing_extensions.is_empty() {
        return Err(format!(
            "Vulkan adapter lacks Ohos shared-frame extensions: {}",
    ...
    let interop = unsafe {
        ...
        OhosVulkanInterop {
            external_memory: OhosExternalMemoryFunctions::load(&raw_instance, raw_device.handle())?,
...
fn required_device_extensions(api_version: u32) -> Vec<&'static CStr> {
    let mut extensions = vec![
        OHOS_NATIVE_BUFFER_EXTENSION_NAME,
        OHOS_EXTERNAL_MEMORY_EXTENSION_NAME,
...
#[cfg(test)]
mod tests {
    use super::{next_descriptor_budget, ohos_custom_vulkan_instance_flags};
    ...
    fn custom_vulkan_instance_disables_debug_utils_but_keeps_validation() {
        ...
        let actual = ohos_custom_vulkan_instance_flags(requested);
