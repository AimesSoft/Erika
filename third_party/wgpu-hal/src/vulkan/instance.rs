...
#[cfg(target_env = "ohos")]
type CreateSurfaceOhos = unsafe extern "system" fn(
    vk::Instance,
...
impl super::Instance {
    ...
    fn create_surface_ohos(
        ...
        let create_surface = unsafe {
            let address = self.shared.entry.get_instance_proc_addr(
                self.shared.raw.handle(),
                c"vkCreateSurfaceOHOS".as_ptr(),
            );
            let Some(address) = address else {
                return Err(crate::InstanceError::new(String::from(
                    "Vulkan driver did not expose vkCreateSurfaceOHOS",
                )));
            };
            core::mem::transmute::<unsafe extern "system" fn(), CreateSurfaceOhos>(address)
        };
        let info = SurfaceCreateInfoOhos {
            s_type: vk::StructureType::from_raw(1_000_685_000),
    ...
    pub unsafe fn init_with_callback(
        ...
        let entry = unsafe {
            ...
            #[cfg(target_env = "ohos")]
            {
                // ash treats the OHOS target as generic Unix and therefore
                // tries libvulkan.so.1. The OpenHarmony NDK/runtime exposes
                // the loader as libvulkan.so.
    ...
    unsafe fn create_surface(
        ...
        match (window_handle, display_handle) {
            ...
            (Rwh::OhosNdk(handle), _) => {
                self.create_surface_ohos(handle.native_window.as_ptr())
            }
            (Rwh::Win32(handle), _) => {
